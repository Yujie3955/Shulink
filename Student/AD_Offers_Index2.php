...
<?php
$query_Cate = sprintf("SELECT * FROM offers_item where OffersItem_Reason = %s ",GetSQLValueString("班代","text"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "Form_Add") && (isset($_POST['Member_ID'])&&$_POST['Member_ID']<>'')) {
	if(isset($_POST['del'])&&$_POST['del']=="刪除"){
		...
		if(isset($_POST['Member_ID'])&&$_POST['Member_ID']<>''){
			//判斷是否有新增過OP
			$query_CateP = sprintf("SELECT * FROM offers where CourseID_Leader = %s and Member_ID=%s and (Course_ID is null and SignupItem_ID is null and SignupRecord_ID is null)",GetSQLValueString($_POST['Course_ID'],"int"),GetSQLValueString($_POST['Member_ID'],"int"));
			$CateP = mysql_query($query_CateP, $dbline) or die(mysql_error());
			$row_CateP = mysql_fetch_assoc($CateP);
			$totalRows_CateP = mysql_num_rows($CateP);
			if($totalRows_CateP<1){
				...
				$insertSQL = sprintf("INSERT INTO Offers (Member_ID, Offers_Money, Offers_Reason, Season_Code, Add_Time, Add_Account, Add_Unitname, Add_Username,CourseID_Leader) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
<?php } ?>
      ...
      <form ACTION="<?php echo $editFormAction; ?>" name="Form_Add" id="Form_Add" method="POST">
        ...
        <fieldset>
              ...
              <span class="FormTitle02 display-inline">分校:
              ...
              <input id="Offers_Money" name="Offers_Money"  type="hidden" value="200">
              </span>
			  ...
			  function callByAJAX3(){//上課學員
						...
						xmlhttp_subitems_member.open("get", "signuprecord_value.php?Course_ID="+ encodeURI(mainItemValue)+"&date="+new Date().getTime(), true);
        ...
        <input type="hidden" name="MM_insert" value="Form_Add" />
             ...
             <?php }else{ ?><br><br><br>
                      <div class="Error_Msg Error_Code center"><img src="../../Icon/delete.gif" alt="錯誤訊息" class="middle"> 優惠項目無"班代"，請先新增！</div>    
                      <?php } 
			 ?> 
      <?php }else{ ?><br><br><br>
