...
if(isset($_GET['orders']) && preg_match("/年齡/i",$_GET['orders']) ){
	...
	$query_Data = sprintf("SELECT * FROM member_detail_nodata WHERE  Member_Show=1 and Com_ID like %s and (ifnull(Member_Username,'') like %s or ifnull(Member_Identity,'') Like %s) order by Member_Birthday ".$orders.", HEX(CONVERT(Member_UserName using big5)) asc",GetSQLValueString($colname03_Unit, "text"),GetSQLValueString($colname_Data2, "text"),GetSQLValueString($colname_Data2, "text"));
}
else{
	$query_Data = sprintf("SELECT * FROM member_detail_nodata WHERE  Member_Show=1 and Com_ID like %s and (ifnull(Member_Username,'') like %s or ifnull(Member_Identity,'') Like %s) order by Com_ID ASC,Member_ID DESC",GetSQLValueString($colname03_Unit, "text"),GetSQLValueString($colname_Data2, "text"),GetSQLValueString($colname_Data2, "text"));
}
...
<div>   
    ...
    <table width="90%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td>
<div class="SubTitle"><img src="../../Icon/IconEdit.png" width="28" class="middle"> <?php echo $row_ModuleSet['ModuleSetting_Title']?>管理:資料未完整</div>
      ...
      <div class="display-inline">姓名、身分證:</div><input type="text" name="Member_UserName" id="Member_UserName" value="<?php if(isset($_GET['Member_UserName'])){echo $_GET['Member_UserName'];} ?>" placeholder="請輸入姓名、身分證關鍵字"> <input type="submit" value="查詢" class="Button_General">
        ...
        <table width="95%" border="0" cellpadding="5" cellspacing="0" class="stripe"> 
          <tr class="TableBlock_shadow_Head_Back">
            ...
            <td class="middle center" width="5%">性別</td> 
            <td class="middle center" width="8%">啟用</td>
            	
            <td class="middle center" width="8%">審核通過</td>
...
return $age; 
			...
			<?php do { ?>
              <tr>
                ...
                <td class="middle center"><?php echo $row_Data['Member_ID']; ?></td>
                ...
                <td class="center middle">
                <?php echo $row_Data['Member_Sex']; ?></td>
                <td class="middle center" ><img src="../../Icon/<?php echo $row_Data['Member_Enable'];?>.png" /></td>
                
                
                 
                <td class="middle">
                <img src="../../Icon/<?php echo $row_Data['Member_Audit'];?>.png" />             
                </td>
                <td class="middle">
