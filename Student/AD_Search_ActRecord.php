...
$query_Data = sprintf("SELECT * FROM study inner join activity on activity.Activity_ID = study.Activity_ID left join activity_cate on activity.ActivityCate_ID=activity_cate.ActivityCate_ID inner join course on course.Course_ID= activity.Course_ID where $Member_IDS and ifnull(course.Com_ID,'') Like %s and ifnull(course.Season_Code,'') Like %s and Study_Attend=1  order by course.Season_Code DESC, study.Activity_ID desc",GetSQLValueString($colname_Data3, "text"),GetSQLValueString($colname_Data2, "text"));
...
<div>   
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td>
<div class="SubTitle"><img src="../../Icon/IconEdit.png" width="28" class="middle"> 戶外教學與社區回饋紀錄</div>
...
<?php } ?>
    ...
    <table width="95%" border="0" cellpadding="5" cellspacing="2">
      <tr>
      <td><input type="button" value="友善列印"  onClick="window.open('AD_Search_SignRecord_Print.php?Season_Code=<?php if(isset($_GET['Season_Code'])&&$_GET['Season_Code']<>""){echo $_GET['Season_Code'];}if(isset($_GET['Com_ID'])&&$_GET['Com_ID']<>""){echo '&Com_ID='.$_GET['Com_ID'];}/*if(isset($_GET['Unit_ID'])&&$_GET['Unit_ID']<>""){echo '&Unit_ID='.$_GET['Unit_ID'];}if(isset($_GET['Course_Name'])&&$_GET['Course_Name']<>""){echo '&Course_Name='.$_GET['Course_Name'];}*/?>')"  class="Button_General">      
      </td>
     ...
     <input type="button" value="全部顯示"  onClick="location.href='<?php echo @$_SERVER["PHP_SELF"];?>'"  class="Button_General">
        ...
        <table width="95%" border="0" cellpadding="5" cellspacing="0" class="stripe"> 
          <tr class="TableBlock_shadow_Head_Back">
            ...
            <td width="15%" class="center middle">時間</td>
		   ...
		   ?>
			<?php do { ?>
              <tr>
                ...
                <?php echo $row_Data['ActivityCate_Name']; ?>
                </td>
                
                <td class="center middle">
                <?php
			if($row_Data['Acticvity_StartTime']<>""){echo date("Y-m-d H:i",strtotime($row_Data['Acticvity_StartTime'])).'至';}if($row_Data['Acticvity_EndTime']<>""){echo date("Y-m-d H:i",strtotime($row_Data['Acticvity_EndTime']));}?></td>
                
                <td class="middle">
                <?php echo $row_Data['Activity_Location']; ?></td>
