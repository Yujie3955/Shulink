...
if(isset($_GET['orders']) && preg_match("/年齡/i",$_GET['orders']) ){
	...
	$query_Data = sprintf("SELECT * FROM member_list WHERE  Member_Show=1  and ifnull(Com_ID,'') like %s and (Member_Username like %s or Member_Identity like %s) and Member_Audit=0 order by Member_Birthday ".$orders.", HEX(CONVERT(Member_UserName using big5)) asc",GetSQLValueString($colname03_Unit, "text"),GetSQLValueString($colname_Data2, "text"),GetSQLValueString($colname_Data2, "text"));	
}
else{
	$query_Data = sprintf("SELECT * FROM member_list WHERE  Member_Show=1  and ifnull(Com_ID,'') like %s and (Member_Username like %s or Member_Identity like %s) and Member_Audit=0 order by Com_ID ASC",GetSQLValueString($colname03_Unit, "text"),GetSQLValueString($colname_Data2, "text"),GetSQLValueString($colname_Data2, "text"));
}
...
<div>   
    ...
    <table width="90%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td>
<div class="SubTitle"><img src="../../Icon/IconEdit.png" width="28" class="middle"> <?php echo $row_ModuleSet['ModuleSetting_Title']?>管理:未通過</div>
...
<?php } ?>
 ...
 <?php } ?>
      <?php if($row_Permission['Per_View']==1){?>
      <input type="button" value="審核通過" class="Button_Add" onClick="location.href='AD_Data_Index.php'"/>
      <input type="button" value="資料未完整" class="Button_Add" onClick="location.href='AD_Data_Noalready.php'"/>
      <input type="button" value="郵寄清單" class="Button_Add" onClick="window.open('AD_Data_Mail.php')"/>
      <?php }?>
    </td>
      ...
      <select name="Com_ID" id="Com_ID" >
        ...
        <?php do { ?>
        <option value="<?php echo $row_Cate['Com_ID']; ?>" <?php if (isset($_GET['Com_ID'])&&$_GET['Com_ID'] == $row_Cate['Com_ID']) { echo "selected='selected'"; } ?>><?php echo mb_substr($row_Cate['Com_Name'],0,3,"utf-8") ?></option>
      ...
      <div class="display-inline">姓名、身分證:</div><input type="text" name="Member_UserName" id="Member_UserName" value="<?php if(isset($_GET['Member_UserName'])){echo $_GET['Member_UserName'];} ?>" placeholder="請輸入姓名、身分證關鍵字"> <input type="submit" value="查詢" class="Button_General">
        ...
        <table width="95%" border="0" cellpadding="5" cellspacing="0" class="stripe"> 
          <tr class="TableBlock_shadow_Head_Back">
            ...
            <td class="middle center" width="20%">身分證</td>
            <td class="middle center" width="8%">身分別</td>	
...
return $age; 
			...
			<?php do { ?>
              <tr>
                ...
                <?php echo $row_Data['Member_ID']; ?></td>
                <td class="center middle Black">
                ...
                <td class="middle">
                  ...
                  <form name="form_Del" id="form_Del" method="POST" action="<?php echo @$_SERVER["PHP_SELF"];?>" class="center">
                    ...
                    <input type="button" value="修改" class="Button_Edit" onClick="location.href='AD_Data_Edit.php?ID=<?php echo $row_Data['Member_ID']; ?>'"/>
                    <?php } ?>
		   ...
		   <table border="0">
                    <tr>
                        ...
                        <?php if ($pageNum_Data > 0) { // Show if not first page ?>
                            <input onClick="location.href='<?php printf("%s?pageNum_Data=%d%s", @$currentPage, 0, $queryString_Data); ?>'" class="gotopage Button_General" type="button" value="第一頁">
                        <?php } // Show if not first page ?>
                        <?php if ($pageNum_Data > 0) { // Show if not first page ?>
                            <input onClick="location.href='<?php printf("%s?pageNum_Data=%d%s", @$currentPage, max(0, $pageNum_Data - 1), $queryString_Data); ?>'" class="gotopage Button_General"  type="button" value="上一頁">
                        <?php } // Show if not first page ?>
                        <?php for($ii=@$min_page;$ii<=@$max_page;$ii++){ ?>
                            <?php if ($ii == @$now_page) { ?>
                                <span class="nowpage"><input value="<?php echo ($ii+1); ?>" class="gotopage Navi_Use" type="button" /></span>
                            <?php } else { ?>
                                <input onClick="location.href='<?php printf("%s?pageNum_Data=%d%s", @$currentPage, $ii, $queryString_Data); ?>'" class="gotopage Navi_NoUse" type="button" value="<?php echo ($ii+1); ?>">
                            <?php } ?>
                        <?php } ?>
                        <?php if ($pageNum_Data < $totalPages_Data) { // Show if not last page ?>
                            <input onClick="location.href='<?php printf("%s?pageNum_Data=%d%s", @$currentPage, min($totalPages_Data, $pageNum_Data + 1), $queryString_Data); ?>'" class="gotopage Button_General" type="button" value="下一頁">
                        <?php } // Show if not last page ?>
                        <?php if ($pageNum_Data < $totalPages_Data) { // Show if not last page ?>
                            <input onClick="location.href='<?php printf("%s?pageNum_Data=%d%s", @$currentPage, $totalPages_Data, $queryString_Data); ?>'" class="gotopage Button_General" type="button" value="最後一頁">
                        <?php } // Show if not last page ?>
                        </td>
                    </tr>
                </table>
