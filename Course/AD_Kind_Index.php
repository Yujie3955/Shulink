...
//主類別資料
$SQL_MainData=sprintf("SELECT * FROM course_kind WHERE CourseKind_ID=%s",
  GetSQLValueString($Col1, "text")
);
$MainData = mysql_query($SQL_MainData, $dbline) or die(mysql_error());
$row_MainData = mysql_fetch_assoc($MainData);
$totalRows_MainData = mysql_num_rows($MainData);

//子類別資料
$query_Cate = sprintf("SELECT * FROM course_kindcate WHERE CourseKind_Code=%s AND ifnull(Com_ID,'') like %s",
    GetSQLValueString($row_MainData['CourseKind_Code'], "text"),
    GetSQLValueString($row_MainData['Com_ID'], "text")
);
...
$totalRows_CateP = mysql_num_rows($CateP);
	...
	$updateSQL = sprintf("UPDATE course_kindcate SET CourseKindCate_Name=%s, CourseKindCate_Sort=%s, Edit_Time=%s, Edit_Account=%s, Edit_Unitname=%s, Edit_Username=%s WHERE CourseKindCate_ID=%s",
                       GetSQLValueString($_POST['CourseKindCate_Name'], "text"),
                       GetSQLValueString($_POST['CourseKindCate_Sort'], "int"),
  ...
  $updateGoTo = @$_SERVER["PHP_SELF"]."?ID=".$_GET['ID']."&Msg=UpdateOK";  
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "Form_Add")) {
	...
	$insertSQL = sprintf("INSERT INTO course_kindcate (Com_ID, CourseKindCate_Name, CourseKindCate_Sort, CourseKind_Code,CourseKindCate_Enable, Add_Time, Add_Account, Add_Unitname, Add_Username) VALUES (%s, %s, %s, %s, %s, %s ,%s ,%s ,%s)",
                       GetSQLValueString($row_MainData['Com_ID'], "int"),
                       GetSQLValueString($_POST['CourseKindCate_Name'], "text"),
                       GetSQLValueString($_POST['CourseKindCate_Sort'], "int"),
                       GetSQLValueString($row_MainData['CourseKind_Code'], "text"),
...
if ((isset($_POST['ID'])) && ($_POST['ID'] != "") && (isset($_POST['Del']))) {
  ...
  $deleteSQL = sprintf("DELETE FROM course_kindcate WHERE CourseKindCate_ID=%s",
                       GetSQLValueString($_POST['ID'], "int"));

  mysql_select_db($database_dbline, $dbline);
  $Result1 = mysql_query($deleteSQL, $dbline) or die(mysql_error());  
  $updateGoTo = @$_SERVER["PHP_SELF"]."?ID=".$_GET['ID']."Msg=DelOK";
  header(sprintf("Location: %s", $updateGoTo));
}

//社區大學資料
$SQL_CommunityData=sprintf("SELECT * FROM community");
$CommunityData = mysql_query($SQL_CommunityData, $dbline) or die(mysql_error());
$row_CommunityData = mysql_fetch_assoc($CommunityData);
$totalRows_CommunityData = mysql_num_rows($CommunityData);
if($totalRows_CommunityData>0){
    $Arr_Community=array();
    do{
        $Arr_Community[$row_CommunityData['Com_ID']]=$row_CommunityData['Com_Name'];
    }while($row_CommunityData = mysql_fetch_assoc($CommunityData));
}
?>
...
<div>   
    ...
    <table width="90%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td>
          <div class="SubTitle"><img src="../../Icon/IconEdit.png" width="28" class="middle"><?php echo $row_MainData['CourseKind_Name']; ?> - <?php echo $row_ModuleSet['ModuleSetting_Title'];  ?></div>
          <table style="width:100%;">
              <tr>
                  <td>
                    <input type="button" class="Button_Add" value="回上一頁" onclick="location.href='AD_Cate_Index.php'">                  
                  </td>
                  <td class="right"></td>
              </tr>
            </table>
      ...
      <form ACTION="<?php echo $editFormAction; ?>" name="Form_Add" id="Form_Add" method="POST">
        <div align="center">
          <fieldset style="max-width:800px;">
              ...
              <br/><?php echo $row_ModuleSet['ModuleSetting_SubName'];?>
              名稱：<input name="CourseKindCate_Name" id="CourseKindCate_Name" type="text" id="Title" style="width:50%;">
              排序：<input name="CourseKindCate_Sort" type="text" id="CourseKindCate_Sort" size="2" value="0">
             </span>
			...
			<?php do { ?>
             <form name="form_Edit" id="form_Edit" method="POST" action="<?php echo $editFormAction; ?>">
              <tr>
               ...
               <td class="middle center"><?php echo $row_Cate['CourseKindCate_ID']; ?></td>
              <td class="middle">				
                <?php 
                    //echo mb_substr(substr($row_Cate['Com_Name'],0,strpos($row_Cate['Com_Name'],"社區")),0,4,"utf-8"); 
                    echo $Arr_Community[$row_Cate['Com_ID']]; 
                ?>
                </td>
                <td class="middle"><input name="CourseKindCate_Name" type="text" id="CourseKindCate_Name" style="width:100%;" value="<?php echo $row_Cate['CourseKindCate_Name']; ?>"></td>
                
                <td class="middle"><input name="CourseKindCate_Sort" type="text" id="CourseKindCate_Sort" size="2" value="<?php echo $row_Cate['CourseKindCate_Sort']; ?>"></td> 
                <?php if ($row_AdminMember['Unit_Range'] >= 2) { ?>
                ...
                <?php if($row_Permission['Per_Edit'] == 1){ ?>
                    ...
                    <input type="button" value="子目錄" class="Button_Edit" onclick="location.href='';">
                    <input type="hidden" name="ID" id="ID" value="<?php echo $row_Cate['CourseKindCate_ID']; ?>">
                ...
                <?php if($row_Permission['Per_Del'] == 1){ ?>
                    ...
                    <input type="hidden" name="ID" id="ID" value="<?php echo $row_Cate['CourseKindCate_ID']; ?>">
                    <input type="hidden" name="Title" id="Title" value="<?php echo $row_Cate['CourseKindCate_Name']; ?>">
                 <?php } ?> 
