...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "Form_Edit_Status1")) {
	if(isset($_POST['sub_check1']) && $_POST['sub_check1']=='通過'){
		...
		*/
		//課程教師資訊
		$Teachers_ID=explode(',',$row_Data['Teacher_ID']);
		//調取教師資料
		$SQL_AllTeacher=sprintf('SELECT * FROM teacher');
		$AllTeacher = mysql_query($SQL_AllTeacher, $dbline) or die(mysql_error());
		$row_AllTeacher = mysql_fetch_assoc($AllTeacher);
		$totalRows_AllTeacher = mysql_num_rows($AllTeacher);
		if($totalRows_AllTeacher>0){
			$Arr_TeacherData=array();
			do{
				$Arr_TeacherData[$row_AllTeacher['Teacher_ID']]= $row_AllTeacher['Teacher_Email'];
 			}while($row_AllTeacher = mysql_fetch_assoc($AllTeacher));
 		}
		
		//引入寄送文件檔案
		require_once("../../Tools/PHPMailer/_acp-ml/modules/phpmailer/class.phpmailer.php");
		require('FailMail.php');
		exit();
		
		//引入檔案(不得寫入once，因有多名老師)
		//$Audit='初審';
