...
mysql_select_db($database_dbline, $dbline);
$query_Data = sprintf("SELECT * FROM course_list_h WHERE ifnull(Season_Code,'') Like %s and (ifnull(CourseKind_Name,'') Like %s or ifnull(Course_Name,'') Like %s) and ifnull(Com_ID,'') like %s and ifnull(Unit_ID,'') like %s  and Course_Check1 like %s and Course_Check2 like %s and Course_Check3 like %s ".$is_teacher." ORDER BY Season_Code DESC, Add_Time DESC",GetSQLValueString($colname02_Data, "text"),GetSQLValueString($colname03_Data, "text"),GetSQLValueString($colname03_Data, "text"), GetSQLValueString($colname03_Unit, "text"), GetSQLValueString($colname04_Data, "text"), GetSQLValueString($colname05_Data, "text"), GetSQLValueString($colname06_Data, "text"), GetSQLValueString($colname07_Data, "text"));
...
<div>   
    ...
    <table width="90%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td>
<div class="SubTitle"><img src="../../Icon/IconEdit.png" width="28" class="middle">歷史<?php echo $row_ModuleSet['ModuleSetting_Title']?>管理：複審</div>
...
<?php } ?>
    ...
    <table width="95%" border="0" cellpadding="5" cellspacing="2">
      ...
      <input type="button" value="回上一頁" class="Button_Add" onClick="location.href='AD_Data_Index.php'"/>
      <?php }?></td>
	...
	初審:<select name="Checks1">
      ...
      <select name="Unit_ID" id="Unit_ID" >
	   ...
	   if($row_Data['Course_Day1']<>""){echo $row_Data['Course_Day1'];}echo $row_Data['Course_Time'];echo date("H:i",strtotime($row_Data['Course_Start1']))."~".date("H:i",strtotime($row_Data['Course_End1'])); ?></td> 
