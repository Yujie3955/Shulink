...
$colname05_Data=$colname02_Unit;
if ((isset($_GET['Checks'])) && ($_GET['Checks'] != "")) {
  $colname05_Data = $_GET['Checks'];
}


$query_Data = sprintf("SELECT * FROM courseteacher_list WHERE CourseTeacher_Pass like %s And ifnull(Season_Code,'') Like %s and (ifnull(CourseTeacher_Name,'') Like %s)  and ifnull(Com_ID,'') like %s and ifnull(Unit_ID,'') like %s ORDER BY courseteacher_list.Season_Code DESC, Add_Time DESC",GetSQLValueString($colname05_Data, "text"),GetSQLValueString($colname02_Data, "text"),GetSQLValueString($colname03_Data, "text"), GetSQLValueString($colname03_Unit, "text"), GetSQLValueString($colname04_Data, "text"));
...
/*查詢刪除資料*/
mysql_select_db($database_dbline, $dbline);
$query_DelData = sprintf("SELECT * from courseteacher_detail WHERE CourseTeacher_ID=%s",GetSQLValueString($_POST['ID'], "int"));
...
$PastContent=$row_DelData['CourseTeacher_ID']."/".$row_DelData['Com_ID']."/".$row_DelData['Unit_ID']."/".$row_DelData['Season_Code']."/".$_POST['CourseKind_ID']."/".$_POST['CourseKind_Name']."/".$row_DelData['CourseTeacher_Name']."/".$row_DelData['CourseTeacher_Credit']."/".$row_DelData['Loc_ID']."/".$row_DelData['Loc_Name']."/".$row_DelData['CourseTeacher_Week']."/".$row_DelData['CourseTeacher_Day']."/".$row_DelData['CourseTeacher_Start']."/".$row_DelData['CourseTeacher_End']."/".$row_DelData['CourseTeacher_Hour']."/".$row_DelData['CourseTeacher_Aims']."/".$row_DelData['CourseTeacher_Summary']."/".$row_DelData['CourseTeacher_Evaluation']."/".$row_DelData['CourseTeacher_Min']."/".$row_DelData['CourseTeacher_Max']."/".$row_DelData['CourseTeacher_Private']."/".$row_DelData['CourseTeacher_Require']."/".$row_DelData['CourseTeacher_Book']."/".$row_DelData['CourseTeacher_Item']."/".$row_DelData['CourseTeacher_Benefit']."/".$row_DelData['CourseTeacher_Pass']."/".$row_DelData['CourseTeacher_Pay']."/".$row_DelData['CourseTeacher_Schedule']."/".$row_DelData['CourseTeacher_Reviews']."/".$row_DelData['Add_Time']."/".$row_DelData['Add_Account']."/".$row_DelData['Add_Unitname']."/".$row_DelData['Add_Username']."/".$row_DelData['Edit_Time']."/".$row_DelData['Edit_Account']."/".$row_DelData['Edit_Unitname']."/".$row_DelData['Edit_Username']."/講師:".$row_DelData['Teacher_UserName']."(".$row_DelData['Teacher_ID'].")";

  require_once('../../Include/Data_BrowseDel.php');
/*刪除*/
 $deleteSQL = sprintf("DELETE FROM course_teacher WHERE CourseTeacher_ID=%s",
                       GetSQLValueString($_POST['ID'], "int"));
 $deleteSQL2 = sprintf("DELETE FROM teacher_nrecord WHERE CourseTeacher_ID=%s",
                       GetSQLValueString($_POST['ID'], "int"));
 $deleteSQL3 = sprintf("DELETE FROM courseteacher_pay WHERE CourseTeacher_ID=%s",
                       GetSQLValueString($_POST['ID'], "int"));
...
<div>   
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td>
<div class="SubTitle"><img src="../../Icon/IconEdit.png" width="28" class="middle"> <?php echo $row_ModuleSet['ModuleSetting_Title']?>管理：教師投課<?php if(isset($_GET['Checks']) && $_GET['Checks']==0){echo '未審區';}elseif(isset($_GET['Checks']) && $_GET['Checks']==1){echo '審核通過';}elseif(isset($_GET['Checks']) && $_GET['Checks']==2){echo '保留區';}?></div>
...
<?php } ?>
    ...
    <table width="95%" border="0" cellpadding="5" cellspacing="2">
	  ...
	  <?php if($row_Permission['Per_View']==1){?>
      <input type="button" value="未審核" class="Button_Add" onClick="location.href='AD_Data_Check.php?Checks=0'"/>
      <input type="button" value="審核通過" class="Button_Add" onClick="location.href='AD_Data_Check.php?Checks=1'"/>
      <input type="button" value="保留區" class="Button_Add" onClick="location.href='AD_Data_Check.php?Checks=2'"/>
     
      <input type="button" value="歷史未審核" class="Button_Add" onClick="location.href='AD_Data_CheckH.php?Checks=0'"/>
      <input type="button" value="歷史審核通過" class="Button_Add" onClick="location.href='AD_Data_CheckH.php?Checks=1'"/>
      <input type="button" value="歷史保留區" class="Button_Add" onClick="location.href='AD_Data_CheckH.php?Checks=2'"/>
      ...
      <select name="Unit_ID" id="Unit_ID" >
        ...
        <div class="display-inline">標題:</div><input type="text" name="<?php echo $row_ModuleSet['ModuleSetting_Code']; ?>_Title" id="<?php echo $row_ModuleSet['ModuleSetting_Code']; ?>_Title" value="<?php echo @$_GET['Course_Title']; ?>" placeholder="請輸入標題關鍵字"> 
         <input type="hidden" name="Checks" value="<?php if(isset($_GET['Checks']) && $_GET['Checks']<>""){echo $_GET['Checks'];}?>">
        ...
        <table width="95%" border="0" cellpadding="5" cellspacing="0" class="stripe"> 
          <tr class="TableBlock_shadow_Head_Back">
            ...
            <td class="middle center" width="15%">操作</td>
          </tr>
			...
			<?php do { ?>
              <tr>
                ...
                <td class="middle Black">
                  <a href="javascript:newin(900,700,'AD_Data_Detail_Check.php?ID=<?php echo $row_Data['CourseTeacher_ID'];?>&Course_Audit=<?php echo $row_Data['CourseTeacher_Pass'];?>')"><?php echo mb_substr($row_Data['CourseTeacher_Name'],0,20,'utf-8');if(mb_strlen($row_Data['CourseTeacher_Name'],'utf-8')>20){echo '...';}?></a>
                </td>
                <td class="middle center"><?php $weekname=explode(",","一,二,三,四,五,六,日");
	   if($row_Data['CourseTeacher_Day']<>""){
	   echo "星期".$weekname[$row_Data['CourseTeacher_Day']-1];}echo $row_Data['CourseTeacher_Time']; echo date("H:i",strtotime($row_Data['CourseTeacher_Start']))."~".date("H:i",strtotime($row_Data['CourseTeacher_End'])); ?></td> 
                ...
                <td class="middle">
                  ...
                  <form name="form_Del" id="form_Del" method="POST" action="<?php echo @$_SERVER["PHP_SELF"];?>" class="center">
                    <?php if(($row_AdminMember['Unit_Range']>="1"&&$row_Permission['Per_Edit'] == 1)){ ?>
						<?php if($row_Data['CourseTeacher_Pass']<>1){?>
                        <input type="button" value="修改" class="Button_Edit" onClick="location.href='AD_Data_Edit_Check.php?ID=<?php echo $row_Data['CourseTeacher_ID']; ?>&Course_Audit=<?php echo $row_Data['CourseTeacher_Pass'];?>'"/>
                            
                        <?php }
                          } ?>
                    <?php if(($row_Permission['Per_Del'] == 1&&$row_AdminMember['Account_Account']==$row_Data['Add_Account'])){ ?>
                    ...
                    <input type="hidden" name="ID" id="ID" value="<?php echo $row_Data['CourseTeacher_ID']; ?>">
                    <input type="hidden" name="Title" id="Title" value="<?php echo $row_Data['CourseTeacher_Name']; ?>">
                  </form>
