...
mysql_select_db($database_dbline, $dbline);
$query_Data = sprintf("SELECT * FROM courseteacher_list WHERE CourseTeacher_Pass=2 And ifnull(Season_Code,'') Like %s and (ifnull(CourseTeacher_Name,'') Like %s) and ifnull(Com_ID,'') like %s and ifnull(Unit_ID,'') like %s ORDER BY courseteacher_list.Season_Code DESC, Add_Time DESC",GetSQLValueString($colname02_Data, "text"),GetSQLValueString($colname03_Data, "text"), GetSQLValueString($colname03_Unit, "text"), GetSQLValueString($colname04_Data, "text"));
...
<div>   
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td>
<div class="SubTitle"><img src="../../Icon/IconEdit.png" width="28" class="middle"> <?php echo $row_ModuleSet['ModuleSetting_Title']?>管理：教師投課保留區</div>
...
<?php } ?>
    ...
    <table width="95%" border="0" cellpadding="5" cellspacing="2">
	  ...
	  if($row_Data['CourseTeacher_Day']<>""){ echo "星期".$weekname[$row_Data['CourseTeacher_Day']-1];}echo $row_Data['CourseTeacher_Time'];echo date("H:i",strtotime($row_Data['CourseTeacher_Start']))."~".date("H:i",strtotime($row_Data['CourseTeacher_End'])); ?></td>
