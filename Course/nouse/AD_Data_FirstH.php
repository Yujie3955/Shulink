...
mysql_select_db($database_dbline, $dbline);
$query_Data = sprintf("SELECT * FROM course_firstlist_h WHERE ifnull(Season_Code,'') Like %s and (ifnull(CourseKind_Name,'') Like %s or ifnull(Course_Name,'') Like %s) and ifnull(Com_ID,'') like %s and ifnull(Unit_ID,'') like %s ORDER BY course_firstlist_h.Season_Code DESC, Add_Time DESC",GetSQLValueString($colname02_Data, "text"),GetSQLValueString($colname03_Data, "text"),GetSQLValueString($colname03_Data, "text"), GetSQLValueString($colname03_Unit, "text"), GetSQLValueString($colname04_Data, "text"));
...
<div>   
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td>
<div class="SubTitle"><img src="../../Icon/IconEdit.png" width="28" class="middle">歷史<?php echo $row_ModuleSet['ModuleSetting_Title']?>管理：初審</div>
...
<?php } ?>
    ...
    <table width="95%" border="0" cellpadding="5" cellspacing="2">
      ...
      <input type="button" value="回上一頁" class="Button_Add" onClick="location.href='AD_Data_First.php'"/>
      <?php }?></td>
      ...
      <select name="Unit_ID" id="Unit_ID" >
        ...
        <div align="center">   
          ...
          <div class="Error_Msg DelError" style="display:none;"><img src="../../Icon/delete.gif" alt="成功訊息" class="middle"> 資料刪除失敗</div>
        ...
        <table width="95%" border="0" cellpadding="5" cellspacing="0" class="stripe"> 
          <tr class="TableBlock_shadow_Head_Back">
            ...
            <td class="middle center" width="20%">名稱</td>
            ...
            <td class="middle center" width="8%">狀態</td>
		   ...
		   ?>
			<?php do { ?>
              <tr>
                ...
                <a href="javascript:newin(900,700,'AD_Data_DetailH.php?ID=<?php echo $row_Data['Course_ID'];?>&Course_Audit=<?php echo $row_Data['Course_Audit'];?>')"><?php echo mb_substr($row_Data['Course_Name'],0,15,"utf8");if(mb_strlen($row_Data['Course_Name'],'utf-8')>15){echo '...';}?></a>
                </td>
	   ...
	   if($row_Data['Course_Day']<>""){echo "星期".$weekname[$row_Data['Course_Day']-1];}echo $row_Data['Course_Time'];echo date("H:i",strtotime($row_Data['Course_Start']))."~".date("H:i",strtotime($row_Data['Course_End'])); ?></td> 
       			
                <td class="middle center"><?php echo str_replace(",",",<br/>",$row_Data['Teacher_UserName'])."&nbsp;"; ?></td>
                <td class="middle center"><img src="../../Icon/<?php if($row_Data['Course_Private']=="0"){ echo '1';}else{echo '0';} ?>.png" /></td>
                <td class="middle center"><?php if($row_Data['Course_Audit']==1){echo '已審';}
				elseif($row_Data['Course_Audit']==2){echo '保留';}else{echo '未審';}?></td>
